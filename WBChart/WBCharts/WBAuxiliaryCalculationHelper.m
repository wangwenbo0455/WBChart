//
//  WBAuxiliaryCalculationHelper.m
//  WBChart
//
...
@implementation WBAuxiliaryCalculationHelper

+ (instancetype)shareCalculationHelper {
  static WBAuxiliaryCalculationHelper *helper = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    helper = [[WBAuxiliaryCalculationHelper alloc] init];
  });
  return helper;
}
// 全正高度比值
...
- (CGFloat)calculateThePositiveNegativeProportionOfHeightByTop:(CGFloat)top
                                                        bottom:(CGFloat)bottom
                                                        height:(CGFloat)height {
      NSLog(@"%d",(top >= height) && (height >= bottom));
//  NSCAssert(
//      (top >= height) && (height >= bottom),
//      @"The data must meet the following conditions “bottom < height < top “");
  if (height >= 0) {
